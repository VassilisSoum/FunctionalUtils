...
public class EitherTest {
    ...
    @Test
    void testZipWith() {
        Either<String, Integer> either1 = Either.right(1);
        Either<String, Integer> either2 = Either.right(2);
        Either<String, Integer> zipped = either1.zipWith(either2, Integer::sum);

        assert (zipped.isRight());
        assert (zipped.getRight() == 3);

        Either<String, Integer> leftEither = Either.left("error");
        Either<String, Integer> zippedWithLeft = either1.zipWith(leftEither, Integer::sum);

        assert (zippedWithLeft.isLeft());
        assert (zippedWithLeft.getLeft().equals("error"));
    }
    ...
    void testZip() {
        Either<String, Integer> either1 = Either.right(1);
        Either<String, Integer> either2 = Either.right(2);
        Either<String, Tuple2<Integer, Integer>> zipped = either1.zip(either2);

        assert (zipped.isRight());
        assert (zipped.getRight().equals(new Tuple2<>(1, 2)));

        Either<String, Integer> leftEither = Either.left("error");
        Either<String, Tuple2<Integer, Integer>> zippedWithLeft = either1.zip(leftEither);
    ...
    void testOrElse() {
        Either<String, Integer> either = Either.left("error");
        Either<String, Integer> orElse = either.orElse(Either.right(42));
        assert (orElse.isRight());
        assert (orElse.getRight() == 42);
    }
